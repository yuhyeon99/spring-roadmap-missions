# spring-missions-playground

Spring Framework & Spring Boot 학습을 위한 **미션 기반 실습 저장소**입니다.
입문부터 핵심 원리, MVC, DB, AOP 고급 주제까지 **Spring 관련 모든 미션을 하나의 레포지토리에서 단계적으로 정리하고 기록**하는 것을 목표로 합니다.

각 미션은 **실습 코드 + 문서(README)** 형태로 관리하며
학습 흐름을 따라가며 **개념 → 코드 → 정리**를 반복하는 방식으로 구성되어 있습니다.

---

## 학습 목표

* Spring Boot의 핵심 개념과 동작 원리 이해
* 객체 지향 설계 원칙과 DI / IoC / AOP 체득
* MVC, DB, 트랜잭션, AOP를 실제 코드로 구현
* 미션 단위로 학습 내용을 문서화하여 재사용 가능한 지식으로 정리

---

## 미션 목록 및 진행 현황

아래는 현재 이 레포지토리에서 다루는 Spring 미션 목록입니다.
각 미션은 토글을 열어 **하위 태스크 단위**로 확인할 수 있습니다.

---

<details>
<summary><strong>[백엔드] 스프링 입문</strong></summary>

스프링 부트의 기본 개념과 핵심 기능을 학습합니다.
REST API 구현, MVC 패턴, DI, JPA, 트랜잭션, AOP 등 스프링 부트의 주요 특징을 실습을 통해 익힙니다.

### 하위 테스크

* [x] **[간단한 웹 페이지와 REST API 엔드포인트 구현](docs/mission-01-spring-intro/task-01-hello/README.md)**
* [x] **[MVC 패턴의 이해와 기본 구조 설계](docs/mission-01-spring-intro/task-02-mvc/README.md)**
* [x] **[의존성 주입(DI) 이해하기](docs/mission-01-spring-intro/task-03-di/README.md)**
* [x] **[JPA와 스프링 데이터의 기본 CRUD 구현](docs/mission-01-spring-intro/task-04-jpa/README.md)**
* [x] **[트랜잭션 관리 설정하기](docs/mission-01-spring-intro/task-05-tx/README.md)**
* [x] **[AOP 적용하여 로깅 기능 추가하기](docs/mission-01-spring-intro/task-06-aop/README.md)**

</details>

---

<details>
<summary><strong>[백엔드] 스프링 핵심 원리 - 기본</strong></summary>

스프링 프레임워크의 핵심 원리와 객체 지향 설계를 심층적으로 학습합니다.
DI, IoC, AOP의 동작 원리와 스프링 컨테이너, 빈 생명주기, 스코프 개념을 실습을 통해 이해합니다.

### 하위 테스크

* [x] **[객체 지향 설계 원칙 이해하기](docs/mission-02-spring-core-basic/task-11-object-oriented-design-principles/README.md)**
* [x] **[Spring을 통한 객체 지향 원리 적용 실습](docs/mission-02-spring-core-basic/task-08-object-oriented-principles/README.md)**
* [x] **[스프링 부트를 사용하여 웹 애플리케이션 프로젝트 생성하기](docs/mission-02-spring-core-basic/task-06-spring-boot-web-project/README.md)**
* [x] **[애너테이션을 사용하여 빈 주입하기](docs/mission-02-spring-core-basic/task-01-annotation-injection/README.md)**
* [x] **[인터페이스를 사용하여 의존성 주입하기](docs/mission-02-spring-core-basic/task-14-interface-injection/README.md)**
* [x] **[의존관계 자동 주입 방식 실습(@Autowired, @Qualifier, @Primary)](docs/mission-02-spring-core-basic/task-04-auto-injection/README.md)**
* [x] **[싱글톤 컨테이너 원리 이해하기](docs/mission-02-spring-core-basic/task-10-singleton-container/README.md)**
* [x] **[싱글톤 빈 스코프와 프로토타입 빈 스코프 구현하기](docs/mission-02-spring-core-basic/task-07-singleton-prototype-scope/README.md)**
* [x] **[스프링 웹 스코프를 활용하여 빈 생성하기](docs/mission-02-spring-core-basic/task-13-spring-web-scope/README.md)**
* [x] **[순환 의존성 해결하기](docs/mission-02-spring-core-basic/task-02-circular-dependency/README.md)**
* [x] **[AOP를 사용하여 애스펙트 구현하기](docs/mission-02-spring-core-basic/task-09-aop-aspect/README.md)**
* [x] **[스프링 MVC를 이용하여 간단한 웹 페이지 구현하기](docs/mission-02-spring-core-basic/task-05-spring-mvc-web-page/README.md)**
* [x] **[스프링 부트와 스프링 MVC를 활용하여 웹 애플리케이션 개발하기](docs/mission-02-spring-core-basic/task-12-spring-boot-mvc-application/README.md)**
* [x] **[의존성 주입과 테스트를 위한 Mock 객체 사용](docs/mission-02-spring-core-basic/task-03-mock-object/README.md)**
* [x] **[스프링 핵심 원리를 활용한 간단한 예제 만들기](docs/mission-02-spring-core-basic/task-15-core-principles-demo/README.md)**

</details>

---

<details>
<summary><strong>[백엔드] HTTP 웹 기본 지식</strong></summary>

HTTP와 웹 통신의 기본 개념을 심층적으로 학습합니다.
요청-응답 구조, 메서드, 상태 코드, 헤더, 캐시, HTTP 버전별 차이를 이해합니다.

### 하위 테스크

* [x] **[네트워크 계층 이해하기](docs/mission-03-http-web-basic/task-10-network-layers/README.md)**
* [x] **[URI의 구조 분석하기](docs/mission-03-http-web-basic/task-05-uri-structure-analysis/README.md)**
* [x] **[HTTP 요청과 응답 흐름 이해하기](docs/mission-03-http-web-basic/task-02-http-request-response-flow/README.md)**
* [x] **[HTTP 메서드별 특징 분석](docs/mission-03-http-web-basic/task-01-http-methods/README.md)**
* [x] **[HTTP 메서드의 실용적 활용](docs/mission-03-http-web-basic/task-07-http-methods-practical-usage/README.md)**
* [x] **[HTTP 상태 코드 심화 이해하기](docs/mission-03-http-web-basic/task-08-http-status-code-deep-dive/README.md)**
* [x] **[일반 HTTP 헤더와 보안 헤더 분석](docs/mission-03-http-web-basic/task-03-general-security-headers/README.md)**
* [x] **[HTTP 요청 헤더와 응답 헤더의 역할 분석](docs/mission-03-http-web-basic/task-04-request-response-headers/README.md)**
* [x] **[HTTP 캐시의 동작 원리](docs/mission-03-http-web-basic/task-09-http-cache-principles/README.md)**
* [x] **[HTTP/2와 HTTP/3의 개선점 비교](docs/mission-03-http-web-basic/task-06-http-version-comparison/README.md)**

</details>

---

<details>
<summary><strong>[백엔드] 스프링 MVC</strong></summary>

스프링 MVC의 요청-응답 흐름과 주요 기능을 실습을 통해 학습합니다.
컨트롤러 매핑, 모델/뷰 분리, 검증, 예외 처리, 필터/인터셉터 등을 직접 구현합니다.

### 하위 테스크

* [x] **[스프링 MVC의 요청-응답 흐름 이해](docs/mission-04-spring-mvc/task-04-request-response-flow/README.md)**
* [x] **[DispatcherServlet의 동작 방식 요약](docs/mission-04-spring-mvc/task-08-dispatcher-servlet-summary/README.md)**
* [x] **[애노테이션을 통한 요청 매핑](docs/mission-04-spring-mvc/task-05-request-mapping-annotations/README.md)**
* [x] **[Model과 View 분리하기](docs/mission-04-spring-mvc/task-12-model-view-separation/README.md)**
* [x] **[View Resolver 설정과 활용](docs/mission-04-spring-mvc/task-02-view-resolver/README.md)**
* [x] **[뷰 템플릿을 사용한 데이터 출력](docs/mission-04-spring-mvc/task-11-view-template-data-output/README.md)**
* [x] **[타입 변환기를 통한 사용자 입력 데이터 변환](docs/mission-04-spring-mvc/task-06-type-converter/README.md)**
* [x] **[검증 애노테이션 사용](docs/mission-04-spring-mvc/task-03-validation-annotations/README.md)**
* [x] **[메시지 소스를 통한 다국어 지원 설정](docs/mission-04-spring-mvc/task-07-message-source/README.md)**
* [x] **[예외 처리와 사용자 알림](docs/mission-04-spring-mvc/task-10-exception-handling-user-notification/README.md)**
* [x] **[필터와 인터셉터를 통한 요청 로깅](docs/mission-04-spring-mvc/task-09-request-logging-filter-interceptor/README.md)**

</details>

---

<details>
<summary><strong>[백엔드] 스프링 DB</strong></summary>

스프링의 데이터 접근 기술과 트랜잭션 관리를 학습합니다.
JdbcTemplate, JPA, MyBatis, Spring Data JPA를 실습하며 실무적인 데이터 처리 경험을 쌓습니다.

### 하위 테스크

* [x] **[Spring Boot에서 JPA 사용하여 데이터 CRUD 구현하기](docs/mission-05-spring-db/task-01-jpa-crud/README.md)**
* [x] **[트랜잭션과 Lazy Loading 연관 테스트](docs/mission-05-spring-db/task-02-transaction-lazy-loading/README.md)**
* [x] **[Spring Data JPA를 활용한 간단한 Repository 만들기](docs/mission-05-spring-db/task-03-spring-data-jpa-repository/README.md)**
* [x] **[트랜잭션 롤백을 위한 커스텀 예외 정의](docs/mission-05-spring-db/task-04-custom-exception-rollback/README.md)**
* [x] **[Spring Boot에서 MyBatis와 JPA를 동시에 사용하기](docs/mission-05-spring-db/task-05-mybatis-jpa-together/README.md)**
* [x] **[예외 처리를 통한 데이터 접근 안정성 강화](docs/mission-05-spring-db/task-06-data-access-exception-handling/README.md)**
* [x] **[스프링 JdbcTemplate을 사용한 데이터베이스 조회](docs/mission-05-spring-db/task-08-jdbctemplate-query/README.md)**
* [x] **[트랜잭션 관리 이해 및 적용](docs/mission-05-spring-db/task-09-transaction-management/README.md)**
* [x] **[트랜잭션 Isolation Level 설정 테스트](docs/mission-05-spring-db/task-07-transaction-isolation-level/README.md)**

</details>

---

<details>
<summary><strong>[백엔드] 스프링 핵심 원리 - 고급</strong></summary>

스프링의 고급 개념과 디자인 패턴을 학습합니다.
프록시, 동적 프록시, AOP, ThreadLocal을 활용한 동시성 및 성능 최적화를 다룹니다.

### 하위 테스크

* [x] **[프록시 패턴을 활용한 보안 검증 시스템 구축해보기](docs/mission-06-spring-core-advanced/task-01-proxy-security/README.md)**
* [x] **[동적 프록시를 이용한 트랜잭션 관리 시스템 구현](docs/mission-06-spring-core-advanced/task-02-dynamic-transaction/README.md)**
* [x] **[템플릿 메서드 패턴을 활용한 확장 가능한 시스템 설계](docs/mission-06-spring-core-advanced/task-03-template-method/README.md)**
* [x] **[스프링 AOP 개념 정리](docs/mission-06-spring-core-advanced/task-06-spring-aop-concepts/README.md)**
* [x] **[@Aspect 어노테이션을 활용한 로깅 시스템 구현](docs/mission-06-spring-core-advanced/task-05-aspect-logging/README.md)**
* [x] **[스프링 AOP를 사용한 예외 처리 시스템 구현](docs/mission-06-spring-core-advanced/task-08-aop-exception-handling/README.md)**
* [x] **[AOP 적용 시 성능 최적화 시스템 구현](docs/mission-06-spring-core-advanced/task-04-aop-performance-optimization/README.md)**
* [x] **[쓰레드 로컬을 사용한 데이터베이스 연결 관리](docs/mission-06-spring-core-advanced/task-07-threadlocal-connection-management/README.md)**

</details>

---

## 기술 스택

* Java (JDK 25)
* Spring Boot 4.0.2
* Gradle
* Spring Web

---

## 실행 방법

```bash
./gradlew bootRun
```

애플리케이션 실행 후 기본 접속 주소:

```
http://localhost:8080
```

---

## 문서 구조 안내

각 미션 및 태스크별 학습 내용은 다음 구조로 관리합니다.

```
docs/
 └─ mission-xx-...
    └─ task-xx-...
       └─ README.md
```

* 코드 구현 배경
* 핵심 개념 정리
* 실행 결과 및 캡처
* 느낀 점 / 정리

을 중심으로 작성합니다.
