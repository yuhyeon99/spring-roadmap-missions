...
### 테스트
- 파일: `src/test/java/com/goorm/springmissionsplayground/mission01_spring_intro/task06_aop/LoggingAspectTest.java`
- 방식: Logback `ListAppender`로 로그를 캡처하고 `MemberTxService.findAll()` 호출 시 AOP 로그가 포함됐는지 검증
...
```
> 네트워크/권한 문제로 Gradle 배포판 다운로드가 불가한 환경에서는 테스트 실행이 실패할 수 있습니다.
...
- **로그 백엔드 활용**: Logback `ListAppender`로 로그를 캡처해 테스트에서도 AOP 적용 여부를 검증할 수 있다.
